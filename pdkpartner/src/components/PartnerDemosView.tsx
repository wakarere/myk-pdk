...
import { PartnerNewDemoDrawer } from "./PartnerNewDemoDrawer";
...
export function PartnerDemosView({ orgName, destination }: { orgName: string; destination: string }) {
  ...
  return (
    <div className="flex h-full">
                ...
                {filtered.map((demo) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      {demo.label && <p className="text-xs text-gray-400 mt-0.5">{demo.label}</p>}
                    </td>
                    <td className="px-4 py-3 text-gray-600 text-sm">
                      {DESTINATION_LABELS[demo.destination] ?? demo.destination}
                    </td>
                    <td className="px-4 py-3">
      ...
      {selected && <DemoDetailPanel demoId={selected} onClose={() => setSelected(null)} />}

      {/* New demo drawer */}
      <PartnerNewDemoDrawer
        open={drawerOpen}
        ...
        {logs.length === 0 ? (
          ...
          logs.map((entry, i) => (
            <div key={i} className="text-xs">
              <span className={`font-medium ${entry.level === "error" ? "text-red-600" : entry.level === "warn" ? "text-amber-600" : "text-gray-500"}`}>
                {entry.stage}
              </span>
